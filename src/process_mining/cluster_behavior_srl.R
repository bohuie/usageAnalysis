...
actor_vectors <- df_pruned %>%
  filter(srl_subcategory %in% kept_actions) %>%
  group_by(actor, srl_subcategory) %>%
  summarise(count = n(), .groups = "drop") %>%
  pivot_wider(names_from = srl_subcategory, values_from = count, values_fill = 0)
...
df_plot_ready <- df_clustered %>%
  ...
  select(session_id, time_created, srl_subcategory, cluster, actor, all_of(grade_columns))
