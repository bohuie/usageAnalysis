...
for (pair in grade_pairs) {
  ...
  for (shift in all_shifts) {
    ...
    p <- ggplot(shift_data, aes(x = srl_subcategory, y = n)) +
      geom_bar(stat = "identity", fill = "steelblue", color = "black") +
      geom_text(aes(label = n), vjust = -0.3, size = 3) +
      scale_y_continuous(limits = c(0, max_count * 1.1)) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      labs(title = paste("SRL Frequency -", label, "-", shift),
  ...
  png(paste0("heatmap_", label, ".png"), width = 1400, height = 350)
  pheatmap(as.matrix(heatmap_df),
           main = paste("SRL Frequency by", label, "Group"),
